...
async def stream_generate(
    ...
    """
    system_prompt = (
        "你是一位专业的技术方案撰写专家，擅长对技术规范书原文进行结构化忠实扩写。"
        "你的核心职责是：严格遵循原文的内容结构和层级顺序，对原文每个要点逐一展开叙述，"
        "不添加原文未涉及的内容，不自由发挥原文未提及的主题。"
        "请使用 Markdown 格式输出。"
    )
    ...
    if original_content.strip():
        parts.append(
            f"请严格按照上述原文的内容结构和层级顺序，对本章节进行扩写，要求如下：\n"
            f"1. 内容字数不少于 {target_words} 字\n"
            f"2. 严格遵循原文的章节结构和层级顺序，不得新增原文中没有的章节或主题\n"
            f"3. 对原文每个要点逐一展开，补充具体的实施细节、操作步骤和注意事项\n"
            f"4. 不自由发挥原文未涉及的内容，所有扩写内容必须基于原文要点\n"
            f"5. 保持与原文整体风格一致，使用规范的技术文档语言"
        )
    else:
        parts.append(
            f"原文暂无具体内容，请仅依据章节标题「{section_title}」，按照规范文档风格撰写本章节，要求如下：\n"
            f"1. 内容字数不少于 {target_words} 字\n"
            f"2. 内容范围限定在章节标题所指向的主题范围内，不扩展到其他主题\n"
            f"3. 使用规范的技术文档语言，结构清晰，层次分明"
        )
